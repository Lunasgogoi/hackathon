...
export default function HackathonLanding() {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#121417] text-slate-100">
      <header className="sticky top-0 z-50 border-b border-white/10 bg-[#121417]/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-5 px-5 py-4 sm:px-8 lg:px-10">
          ...
          <nav className="hidden items-center gap-7 text-sm font-bold text-slate-300 lg:flex">
            {navLinks.map((link) => (
              <a key={link.href} href={link.href} className="no-underline transition hover:text-emerald-200 hover:underline">
                {link.label}
              </a>
            ))}
          </nav>
          <div className="flex items-center gap-3">
            <Link to="/auth" className="hidden no-underline text-sm font-bold text-slate-300 transition hover:text-emerald-200 hover:underline sm:inline">
              Login
            </Link>
            <Link
      ...
      <main id="home">
        <section className="border-b border-white/10 bg-[#121417]">
          <div className="mx-auto flex min-h-[calc(100vh-73px)] max-w-7xl items-center px-5 py-16 sm:px-8 lg:px-10">
            <div className="min-w-0 max-w-5xl">
              <div className="mb-8 inline-flex rounded-full border border-emerald-200/25 bg-emerald-200/10 px-4 py-2 text-xs font-bold uppercase tracking-wider text-emerald-100">
                National-Level Coding Competition
              </div>
              <h1 className="max-w-5xl text-4xl font-bold leading-tight tracking-tight text-slate-50 sm:text-6xl">
                Build, compete, and ship under pressure.
              </h1>
              <p className="mt-8 max-w-4xl text-lg font-medium leading-8 text-slate-300 sm:text-xl sm:leading-9">
              ...
              <div className="mt-9 flex flex-col gap-4 sm:flex-row sm:flex-wrap">
                ...
                <Link
                  to="/auth"
                  className="w-full no-underline rounded-md border border-zinc-600 px-7 py-3 text-center text-sm font-bold text-slate-100 transition hover:border-emerald-200 hover:text-emerald-100 hover:no-underline sm:w-auto"
                >
                  Login to Start Test
                </Link>
              </div>
          ...
          <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr]">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              {[
                ...
                <article key={title} className="rounded-lg border border-zinc-700/70 bg-[#1d2127] p-5">
                  <h3 className="text-lg font-bold text-emerald-200">{title}</h3>
                  <p className="mt-3 text-sm font-medium leading-6 text-slate-300">{body}</p>
                </article>
              ))}
            </div>
          </div>
        ...
        <section id="flow" className="scroll-mt-20 bg-[#171a1f]">
          <div className={sectionClass}>
            <div className="mx-auto max-w-4xl rounded-lg border border-zinc-700/70 bg-[#1d2127] p-6 shadow-xl shadow-black/15 sm:p-8">
              <div className="border-b border-zinc-700/70 pb-5">
                <div className="text-sm font-bold uppercase tracking-wider text-emerald-200">Live Event Flow</div>
              ...
              <div className="mt-6 space-y-4">
                {[
                  ...
                  <div key={number} className="flex flex-col gap-4 border border-zinc-700/70 bg-zinc-950/30 p-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <span className="text-xs font-bold uppercase text-slate-400 sm:whitespace-nowrap">{status}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <section id="rounds" className="scroll-mt-20 bg-[#171a1f]">
          <div className={sectionClass}>
            ...
            <div className="mt-12 grid gap-6 lg:grid-cols-3">
              {[
                ...
                <article
                  key={round}
                  className="rounded-lg border border-zinc-700/70 bg-[#1d2127] p-7 transition hover:border-emerald-200"
                >
                  <div className="text-sm font-bold uppercase tracking-wider text-emerald-200">{round}</div>
                  <h3 className="mt-4 text-2xl font-bold">{title}</h3>
                  <p className="mt-4 text-base font-medium leading-7 text-slate-300">{body}</p>
                </article>
              ))}
            </div>
          </div>
        ...
        <section id="prizes" className={`${sectionClass} scroll-mt-20`}>
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-3">
            {[
              ...
              <article key={title} className="rounded-lg border border-zinc-700/70 bg-[#1d2127] p-8 text-center transition hover:border-emerald-200">
                <h3 className="text-2xl font-bold text-emerald-200">{title}</h3>
                <p className="mt-5 text-lg font-bold text-slate-200">{subtitle}</p>
              </article>
            ))}
          </div>
        </section>

        <section id="results" className="scroll-mt-20 bg-[#171a1f]">
          <div className={sectionClass}>
            <h2 className="text-center text-3xl font-bold tracking-tight sm:text-4xl">Event Status & Results</h2>
            <div className="mx-auto mt-10 max-w-3xl rounded-lg border border-zinc-700/70 bg-[#1d2127] p-8 text-center">
              ...
              <Link
                to="/leaderboard"
                className="mt-7 inline-flex no-underline rounded-md border border-emerald-200/80 px-6 py-3 text-sm font-bold text-emerald-100 transition hover:bg-emerald-200 hover:text-emerald-950 hover:no-underline"
              >
                View Leaderboard
              </Link>
            </div>
        ...
        <section id="faq" className={`${sectionClass} scroll-mt-20`}>
          ...
          <div className="mx-auto mt-10 max-w-5xl divide-y divide-zinc-700/70 border-y border-zinc-700/70">
            {questions.map((item) => (
              <details key={item.question} className="group bg-[#1d2127]">
                <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-4 py-7 text-xl font-semibold text-slate-100 sm:px-6">
                  <span>{item.question}</span>
                  <span className="text-2xl font-light text-emerald-200 transition group-open:rotate-45">+</span>
                </summary>
                <p className="px-4 pb-7 text-base font-medium leading-7 text-slate-300 sm:px-6">{item.answer}</p>
              </details>
            ))}
          </div>
        </section>
      ...
      <footer id="contact" className="scroll-mt-20 border-t border-white/10 bg-[#121417]">
        <div className="mx-auto grid max-w-6xl gap-8 px-5 py-14 sm:px-8 md:grid-cols-3 lg:px-10">
          ...
          <div>
            ...
            <div className="mt-4 flex gap-3">
              <Link to="/auth" className="no-underline text-sm font-bold text-slate-300 transition hover:text-emerald-200 hover:underline">Login</Link>
              <Link to="/auth?mode=register" className="no-underline text-sm font-bold text-emerald-200 transition hover:text-emerald-100 hover:underline">Register</Link>
            </div>
          </div>
        </div>
      </footer>
